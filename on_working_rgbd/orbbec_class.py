...
#             if color_frame:
#                 color_frame_bgr = self.frame_to_bgr_image(color_frame)
#                 color_frame_resized = self.resize_frames(color_frame_bgr, scale_percent)
#                 gpu_color = cv2.cuda_GpuMat()
#                 gpu_color.upload(color_frame_resized)
#             else:
#                 gpu_color = None
...
#             depth_data = self.get_depth_data(depth_frame)
#             if depth_data is not None:
#                 depth_data_resized = self.resize_frames(depth_data, scale_percent)
#                 gpu_depth = cv2.cuda_GpuMat()
#                 gpu_depth.upload(depth_data_resized)
#             else:
#                 gpu_depth = None

#             return gpu_color, gpu_depth
...
#     def view_stream(self, show_color=True, show_depth=True, scale_percent=50):
#         while True:
#             try:
#                 color_data, depth_data = self.get_frames(scale_percent)

#                 if show_color and color_data is not None:
#                     color_image = color_data.download()
#                     cv2.imshow("Color Viewer (CUDA)", color_image)

#                 # Affichage profondeur
#                 if show_depth and depth_data is not None:
#                     # Normalisation et mapping des couleurs sur le GPU
#                     depth_normalized = cv2.cuda.divide(
#                         cv2.cuda.subtract(depth_data, self.MIN_DEPTH), (self.MAX_DEPTH - self.MIN_DEPTH)
#                     )
#                     depth_normalized = cv2.cuda.multiply(depth_normalized, 255)
#                     depth_normalized = cv2.cuda.clip(depth_normalized, 0, 255)

#                     # Conversion en uint8
#                     gpu_depth_uint8 = cv2.cuda.convertScaleAbs(depth_normalized)

#                     # Application du colormap
#                     depth_colormap = cv2.applyColorMap(gpu_depth_uint8.download(), cv2.COLORMAP_JET)
#                     cv2.imshow("Depth Viewer (CUDA)", depth_colormap)
...
if __name__ == "__main__":
    orbbec = Orbbec()
    orbbec.view_stream(show_color=False)
    orbbec.stop()
